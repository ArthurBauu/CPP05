...
{
	o << "AForm " << a.getName() 
	<< ", signed: " << (a.isSigned() ? "true" : "false") 
	<< ", grade required to sign: " << a.getGradeToSign() 
	<< ", grade required to execute: " << a.getGradeToExecute() 
	<< "." 
	<< std::endl;
	return o;
}
...
void AForm::execute(Bureaucrat const &executor) const
{
	if (!this->isSigned())
		...
		std::cout << executor.getName() << " cannot execute " << this->getName() << " because their grade is too low." << std::endl;
	else
	{
		this->executeAction();
